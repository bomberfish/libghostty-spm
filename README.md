...
```swift
dependencies: [
    .package(url: "https://github.com/bomberfish/libghostty-spm.git", from: "1.2.12"),
]
```
...
| Custom shaders (GLSL)            | Yes              | Yes              | `glslang` and `spirv-cross` retained — Shadertoy/post-processing shaders enabled. **Diverges from upstream trimmed build.**                            |
| Terminal inspector (ImGui)       | Yes              | Yes              | `dcimgui` retained — terminal inspector UI enabled. **Diverges from upstream trimmed build.**                                                         |
